...
const TRANSFER_CONTEXT_PROMPT =
  "[SYSTEM CONTEXT: You are now handling this conversation after a transfer from another agent. Speak to the user naturally, do not greet them and continue helping them. Do not mention the transfer. Do not transfer back to the previous agent.]";

// RAG configuration constants
const RAG_CONTEXT_MESSAGES = 3; // Number of recent messages to consider for RAG context

/**
 * Sanitizes tool names to meet Google Generative AI requirements:
...
function getIncomingEdgesWithReturn(
  nodeId: string,
  edges: AppEdge[],
): AppEdge[] {
  return edges.filter(
    (e) => e.target === nodeId && e.data?.returnTransition?.enabled,
  );
}
 ...
 */
function getParentAgents(
  agentNodeId: string,
  edges: AppEdge[],
  nodes: AppNode[],
): AppNode[] {
  const incomingFromAgents = edges.filter((e) => e.target === agentNodeId);
 ...
 */
function getConversationMessagesWithoutToolTurns(
  ...
  return messages.filter((msg): msg is HumanMessage | AIMessage => {
    ...
    if (AIMessage.isInstance(msg))
      return !(msg as AIMessage).tool_calls?.length;
    return false;
    ...
    if (targetNode.type === "agent") {
      ...
      const schemaFields: Record<string, z.ZodString> = {
        reason: z
          .string()
          .describe(
            "Brief reason why you are transferring the conversation (e.g. what the user asked for or indicated).",
          ),
      };
      ...
      const toolName = sanitizeToolName(`transfer_to_${targetLabel}`);
      const description =
        edge.data?.conditionExpression ||
        `Transfer conversation to ${targetLabel}`;

      const transferTool = tool(
        (params) => {
          const reason = (params as { reason?: string }).reason;
          const collectedFields = Object.keys(params).filter(
            (k) => k !== "reason",
          );
          const parts = [
            reason ? `Reason: ${reason}` : null,
            collectedFields.length > 0
              ? `Collected: ${collectedFields.join(", ")}`
              : null,
          ].filter(Boolean);
      ...
      const schemaFields: Record<string, z.ZodString> = {
        reason: z
          .string()
          .describe(
            "Brief reason why you are ending the conversation (e.g. user said goodbye, request completed).",
          ),
      };
      if (agentData.infoCollection && agentData.infoCollection.length > 0) {
        agentData.infoCollection.forEach((field) => {
      ...
      const description =
        (targetNode.data as any).description ||
...
function createReturnTools(
  ...
  for (const edge of incomingEdgesWithReturn) {
    ...
    const toolName = sanitizeToolName(`return_to_${sourceLabel}`);
    ...
    const description =
      returnConfig.conditionExpression || defaultReturnDescription;
    const returnTool = tool(
      (params) => {
        const reason = (params as { reason?: string }).reason;
        return reason
          ? `Returned to ${sourceLabel}. Reason: ${reason}`
      ...
      },
      {
        ...
        schema: z.object({
          reason: z
            .string()
            .describe(
              "Brief reason why you are returning (e.g. no other agent can help and you cannot answer; use only when transfer/answer is not possible).",
            ),
        }),
      },
    );
...
function buildAgentConfigMap(
  ...
  for (const agentNode of agentNodes) {
    ...
    const transferToolInfos = createTransferTools(
      agentNode,
      outgoingEdges,
      nodes,
    );
    ...
    const returnToolInfos = createReturnTools(
      agentNode.id,
      incomingWithReturn,
      nodes,
    );
    ...
    const basePrompt =
      agentData.conversationGoal || "You are a helpful assistant.";
    const transferInstructions =
      allTransferTools.length > 0 ? TRANSFER_INSTRUCTIONS_PROMPT : "";
...
function createRagNode(
  ...
  return async (state) => {
    ...
    if (!config) {
      console.warn(
        `⚠️ Agent configuration not found for RAG: ${currentAgentId}`,
      );
      return { messages: [] };
    }
    ...
    if (!config.knowledgeBases || config.knowledgeBases.length === 0) {
      console.log(
        `📚 Agent ${config.label} has no knowledge bases, skipping RAG`,
      );
      return { messages: [] };
    }

    // Use only conversational turns (no tool calls/responses) so Gemini is not sent invalid turn order
    const conversationOnly = getConversationMessagesWithoutToolTurns(
      state.messages,
    );
    ...
    if (
      ...
      !HumanMessage.isInstance(lastConversationMessage)
    ) {
      console.log(
        `⚠️ Last conversational message is not a HumanMessage, skipping RAG`,
      );
      return { messages: [] };
    }
    ...
    const QuestionDetectionSchema = z.object({
      shouldRetrieve: z
        .boolean()
        .describe("Whether this message requires knowledge base lookup"),
      searchQuery: z
        .string()
        .nullable()
        .describe(
          "The optimized search query to use, or null if no retrieval needed",
        ),
      reasoning: z.string().describe("Brief explanation of the decision"),
    });

    const detectionModelWithStructure = detectionModel.withStructuredOutput(
...
When in doubt, it is better to retrieve than to skip. If retrieval is needed, extract the key search terms from the conversation context and create an optimized search query.`,
      ...
      console.log(
        `🤔 RAG Detection: shouldRetrieve=${detection.shouldRetrieve}, query="${detection.searchQuery}", reason="${detection.reasoning}"`,
      );
      ...
      if (!vectorStoreCache[cacheKey]) {
        console.log(
          `🔄 Initializing vector store for agent ${config.label}...`,
        );
        vectorStoreCache[cacheKey] = await initializeVectorStore(
      ...
      const contextText = results.map((r, idx) => `${r.content}`).join("\n\n");
...
${contextText}`;
      ...
      console.log(
        `✨ Augmenting message with ${results.length} context chunks`,
      );
...
function createDynamicAgentNode(
  ...
  return async (state) => {
    ...
    const justTransferred =
      previousAgentId !== null && previousAgentId !== currentAgentId;
    ...
    if (justTransferred) {
      ...
      if (messagesToFilter.size > 0) {
        messagesToSend = state.messages.filter(
          (_, idx) => !messagesToFilter.has(idx),
        );
      }
    }
    ...
    const allTools = [...config.transferTools, ...(config.composioTools ?? [])];
...
function createEndNode(): GraphNode<typeof AgentState> {
  return async (state) => {
    ...
    return {
      messages: [
        new SystemMessage("Session finished. Thank you for using our service."),
      ],
    };
  };
}
  ...
  static async createAgentGraph(
    ...
    const routeAfterTool = (state: any) => {
      ...
      const lastAiMessage = [...state.messages]
        ...
        .find(
          (msg) =>
            AIMessage.isInstance(msg) && (msg as AIMessage).tool_calls?.length,
        );

      const hasTransfer =
        lastAiMessage &&
        (lastAiMessage as AIMessage).tool_calls?.some(
          (tc) => toolToAgentMap[tc.name],
        );
    ...
    for (const agentId of Object.keys(configMap)) {
      ...
      if (config.tools && config.tools.length > 0) {
        try {
          ...
          console.warn(
            `Failed to load Composio tools for agent ${config.label}:`,
            e,
          );
          config.composioTools = [];
        }
    ...
    const workflow = new StateGraph(AgentState)
      ...
      .addConditionalEdges("agentNode", shouldContinueFromAgent, [
        "toolNode",
        END,
      ])
